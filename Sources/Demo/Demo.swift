...
{
  Graf.initialize()
  let v = Graf.newView("static", 640, 480)
  var x = 0
  var y = 120
  
  let e = Graf.ellipse(0, 0, 50, 150)
  //let cb = Graf.Image("./media/ball.png")
  ...
  v.draw
  {
    ...
    dc.line(0, 0, Int16(dc.width), Int16(dc.height))
    dc.fill = .color(Graf.Color(255, 0, 0, 180))
    e.moveTo(Double(x), Double(y))
    e.rotate(0.01)
    e.draw(dc)
  }
